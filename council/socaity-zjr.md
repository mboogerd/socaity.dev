...
### [mechanism-designer]
CONSENT. I confirm the Facilitator's reading: the distribution may execute for unchallenged entries while payouts on challenged entries are withheld until adjudication — exactly what my boundary requires, satisfied entry-by-entry. One mechanical note for drafting, not an objection: the challenged entry's pro-rata share must be **escrowed at declared weight, not redistributed to the unchallenged cohort** — otherwise every participant gains from challenging every rival and the challenge mechanism itself becomes the attack. Escrow-then-release (or escrow-then-discount, remainder flowing per the published rule) keeps point 6's invariant intact. I accept legal's point-5 revision (explicit non-transferability also closes the secondary-market Goodhart path) and point 7 as written.

### [legal-counsel]
CONSENT. I confirm the reconciling reading; for the record, the binding declaration covers only the confirmed set — withheld entries join a later declaration upon confirmation, so finality attaches per declaration, not per cohort. I accept all MD Round-2 additions: stake-or-clock (provided stakes remain reputation-only and "frivolous" = bad-faith, not merely unsuccessful — both already in the standing text); one-round staked appeals; cluster-level art. 22 review (individual-rights compatibility goes to identity-specialist, material before first distribution, not M0-blocking); pre-M3 queue with no platform/founder interim adjudication. Standing reminder: the integrated policy ships inside the socaity-1ux licensed-counsel review together with the epoch-share wording — the two cannot publish separately. (Analysis for decision-making, not legal advice.)

## Resolution

ADOPTED by consent, Round 3, no paramount objections standing.

**The Ledger Validation Policy** (published at M0 as part of the standing commitment; embedded in the socaity-x8o distribution-rule code; reviewed by licensed NL counsel inside socaity-1ux together with the epoch-share wording — the two publish together or not at all):

1. Every ledger entry is born `provisional`; only `confirmed` entries are paid. Status changes are new, signed, public entries citing evidence.
2. Mandatory ex-post audit of all pre-M3 entries by the M3 verification market is a blocking precondition of the first distribution; audit and human-review work funded from a fixed, capped, published slice of the distribution pie (placeholder 5%).
3. Public challenge window (placeholder ≥90 days) between announcement of a *provisional pool* and binding declaration; binding declaration covers only the confirmed set; challenged entries' shares are escrowed at declared weight (never redistributed to the cohort) and join a later declaration upon confirmation.
4. Correlated-identity discounting by an algorithm published at M0; amendable only prospectively via open comment period; automated discounts are proposals confirmed by a named human reviewer at cluster level (GDPR art. 22; individual-rights compatibility check with identity-specialist before first distribution).
5. Confirmation renders eligibility and relative weight final; monetary amounts arise only upon declaration; confirmed entries remain non-transferable; the platform will not initiate recovery of settled distributions.
6. Invariant: expected payout of a fabricated entry ≤ expected cost of fabricating it plus surviving audit and challenge. If adjudication costs exceed the audit slice: raise stakes or batch by cluster, never pay provisional entries to clear the queue.
7. Challenge process: enumerated grounds (fraud, Sybil fabrication, collusion, fabricated edges, misrepresented capacity); reputation-only stakes (burned only on bad-faith, not merely unsuccessful, challenges); notice + ≥30-day response; decided by the verification market (post-split under stichting oversight), never solely by the platform; one staked appeal to a differently-composed panel; all decisions published with evidence. Pre-M3 challenges queue on-ledger with stakes escrowed; no platform/founder interim adjudication.

Follow-ups: (a) socaity-1ux scope now includes this policy text; (b) identity-specialist question on cluster-level art. 22 review to be raised on socaity-9g1 or successor; (c) vocabulary: "validation"/"challenge", never "clawback", in all public text.
